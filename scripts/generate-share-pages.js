...
function render(article){
  ...
  const html = [
    ...
    "<p class='share-note'>リンクをコピーしたり、SNSで簡単に共有できます。</p>",
    "<div class='share-buttons'>",
    "<button id='facebookButton' class='facebook' type='button'>Facebookでシェア</button>",
    ...
    "async function copyValue(value,message){try{await navigator.clipboard.writeText(value);copyStatus.textContent=message;return true;}catch(error){window.prompt('下のリンクをコピーしてください。',value);return false;}}",
    "facebookButton.addEventListener('click',function(){location.href='https://www.facebook.com/sharer/sharer.php?u='+encodeURIComponent(shareUrl)+'&quote='+encodeURIComponent(shareText);});",
